...
function ProxyInstrument({ instrument }: { instrument: MarketCoreProxyGroup["instruments"][number] }) {
  ...
  return <article className="reader-proxy-card">
    ...
    <p className="reader-proxy-time">{instrument.live.status === "available" ? `${instrument.live.display_mode === "same_day_session_latest" ? `${liveMessage(instrument.live)}；` : ""}行情时间：${formatShanghaiQuoteDateTime(instrument.live.quote_datetime)}` : `${liveMessage(instrument.live)}；展示最近完整收盘。`}</p>
