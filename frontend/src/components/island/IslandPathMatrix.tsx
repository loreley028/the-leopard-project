    ...
    <div className="matrix-desktop matrix-viewport" ref={viewport}>
      <table className="path-matrix"><colgroup><col className="matrix-sector-column" />{matrix.dates.map(item => <col className="matrix-date-column" key={item.trading_date} />)}</colgroup><caption className="sr-only">{matrix.caption}</caption><thead><tr><th className="sticky-sector" scope="col">板块</th>{matrix.dates.map((item, index) => <th className={index === matrix.dates.length - 1 ? "latest-column" : ""} scope="col" key={item.trading_date} title={`完整交易日 ${item.trading_date} ${item.weekday}`}><span>{shortDate(item.trading_date)} <b>{shortWeekday(item.weekday)}</b></span></th>)}</tr></thead>
        <tbody>{groups.flatMap(({ group_name: group, rows: items }) => [
          ...
          ...items.map(row => <tr key={row.sector_key}><th className="sticky-sector" scope="row"><Link to={`/sectors/${row.sector_key}`}>{row.sector_name}</Link></th>{row.cells.map((cell, index) => <td key={cell.trading_date} className={index === row.cells.length - 1 ? "latest-column" : ""}><button type="button" className={`path-cell ${cell.path_status ? `path-${cell.path_status}` : "path-no-report"}`} style={cell.path_status_color ? { backgroundColor: cell.path_status_color } : undefined} onClick={() => setSelected({ ...cell, sector_name: row.sector_name, sector_key: row.sector_key })} aria-label={cellAriaLabel(row.sector_name, cell)} title={marketOverlayTitle(cell)}><span>{reportStatus(cell)}</span><small className={`market-overlay-${cell.market_overlay?.kind ?? "unavailable"}`}>{marketOverlayLabel(cell)}</small></button></td>)}</tr>),
        ])}</tbody>
      </table>
    </div>
