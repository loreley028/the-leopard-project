...
const reportStatus = (cell: Cell) => cell.report_present ? cell.path_status_label ?? "—" : "未提";
...
export function IslandPathMatrix({ matrix, currentMarket, period, onPeriodChange }: { matrix: PathMatrix; currentMarket: MarketCoreCurrentQuotes | null; period: string; onPeriodChange: (value: string) => void }) {
  ...
  return <div className="path-matrix-shell">
    ...
    <p className="matrix-caption">{matrix.caption}按受控完整交易日排列；颜色为当期报告路径标记，“未提”表示该期报告未单独更新。市场行情仅按该日期精确读取，不做行情数据回退。相关证券逐项表现请点击查看；不构成板块指数或综合收益。</p>
    {matrix.report_calendar?.length ? <p className="matrix-report-calendar" aria-label="报告日历">报告时间线：{matrix.report_calendar.map((item, index) => <span className={item.state === "no_live" ? "report-calendar-no-live" : ""} key={`${item.report_date}-${item.state}`}>{index ? " · " : ""}{shortDate(item.report_date)} {item.display_label}</span>)}</p> : null}
    <div className="matrix-desktop matrix-viewport" ref={viewport}>
      <table className="path-matrix" data-column-model="fixed"><colgroup><col className="matrix-sector-column" /><col className="matrix-current-column" />{matrix.dates.map(item => <col className="matrix-date-column" key={item.trading_date} />)}</colgroup><caption className="sr-only">{matrix.caption}</caption><thead><tr><th className="sticky-sector" scope="col">板块</th><th className="sticky-current matrix-current-heading" scope="col">当前行情<small>{currentMarket ? `${currentMarket.snapshot_ttl_seconds ?? 60}s 刷新` : "加载中"}</small></th>{matrix.dates.map((item, index) => <th className={index === matrix.dates.length - 1 ? "latest-column" : ""} scope="col" key={item.trading_date} title={`完整交易日 ${item.trading_date} ${item.weekday}`}><span className="matrix-date-header">{shortDate(item.trading_date)} <b>{shortWeekday(item.weekday)}</b></span></th>)}</tr></thead>
        <tbody>{groups.flatMap(({ group_name: group, rows: items }) => [
          ...
          ...items.map(row => <tr key={row.sector_key}><th className="sticky-sector" scope="row">{row.market_available === false ? <span>{row.sector_name}</span> : <Link to={`/sectors/${row.sector_key}`}>{row.sector_name}</Link>}</th><td className="sticky-current"><CurrentMarketCell sector={currentBySector.get(row.sector_key)} /></td>{row.cells.map((cell, index) => <td key={cell.trading_date} className={index === row.cells.length - 1 ? "latest-column" : ""}><button type="button" className="path-cell" onClick={() => setSelected({ ...cell, sector_name: row.sector_name, sector_key: row.sector_key, market_available: row.market_available })} aria-label={cellAriaLabel(row.sector_name, cell)} title={marketOverlayTitle(cell)}><span className={`matrix-report-layer ${cell.path_status ? `path-${cell.path_status}` : "path-no-report"}`} style={cell.path_status_color ? { backgroundColor: cell.path_status_color } : undefined}><b>{reportStatus(cell)}</b></span><ExactDateMarketLayer cell={{ ...cell, sector_name: row.sector_name, sector_key: row.sector_key, market_available: row.market_available }} /></button></td>)}</tr>),
        ])}</tbody>
      </table>
    </div>
    <div className="matrix-mobile" aria-label="移动端板块最近五个交易日行情">{groups.map(({ group_name: group, rows: items }) => <section className="matrix-mobile-group" data-group-name={group} ref={node => { if (node) mobileGroups.current.set(group, node); else mobileGroups.current.delete(group); }} key={group}><h3>{group}</h3>{items.map(row => <details key={row.sector_key}><summary>{row.sector_name}</summary><ol>{row.cells.slice(-5).map(cell => <li key={cell.trading_date}><time>{cell.trading_date}</time><button type="button" className="matrix-mobile-path-tile" onClick={() => setSelected({ ...cell, sector_name: row.sector_name, sector_key: row.sector_key })}><span className={`matrix-report-layer ${cell.path_status ? `path-${cell.path_status}` : "path-no-report"}`} style={cell.path_status_color ? { backgroundColor: cell.path_status_color } : undefined}>{reportStatus(cell)}</span><ExactDateMarketLayer cell={{ ...cell, sector_name: row.sector_name, sector_key: row.sector_key }} /></button></li>)}</ol></details>)}</section>)}</div>
