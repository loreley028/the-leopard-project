...
const liveMarketAnchor = { market_context: "live_market_anchor" as const, market_context_note: "当前市场辅助：实时上证指数用于对照本报告攻防线，不代表报告日期的历史指数。", quote_status: "available" as const, symbol: "sh000001" as const, index_name: "上证指数", current: 3856.47, pre_close: 3842.12, change: 14.35, pct_change: .37, quote_datetime: "2026-08-11T13:42:00+08:00", provider: "tencent_standard_security_quote", provider_role: "diagnostic_provider", error_code: null, cache_hit: false, defense_line_value: 3844, defense_line_source: "core_view" as const, stand_above_condition: "即使站上，也必须通过时间、市场宽度和量能验证。", break_below_condition: "3844点以下继续防守。", validation_conditions: "即使站上，也必须通过时间、市场宽度和量能验证。", distance_points: 12.47, distance_pct: .32, defense_position: "above_defense_line" as const };
const enhanced: EnhancedReport = { report, path_entries: [pathEntry], sector_assessments: [assessment], status_groups: [{ status: "hold", count: 1, items: [assessment] }], market_snapshots: [], live_market_anchor: liveMarketAnchor, recent_defense_line_validations: [], comparison: { previous_report_id: null, status_changes: [], counts: {} }, market_data_attached: false, data_notice: report.data_notice };
...
describe("Viewer research pages", () => {
  ...
  it("shows current market assistance separately from the historical report and structures its defense line", async () => { renderAt("/"); expect(await screen.findByRole("heading", { name: report.title })).toBeInTheDocument(); expect(screen.getByText(report.core_view)).toBeInTheDocument(); expect(screen.getByText("猎豹核心判断")).toBeInTheDocument(); expect(screen.getByText("大盘观察 / 市场锚点")).toBeInTheDocument(); expect(screen.getByText("当前市场辅助")).toBeInTheDocument(); expect(screen.getByText("上个交易日收盘")).toBeInTheDocument(); expect(screen.getByText("今日上证指数")).toBeInTheDocument(); expect(screen.getAllByText("3,844.00")).toHaveLength(2); expect(screen.queryByText(/3,844\.00 点/)).not.toBeInTheDocument(); expect(screen.getByText("攻防线上方")).toHaveClass("a-share-positive"); expect(screen.getByText(/核心判断安全回退/)).toBeInTheDocument(); });
  it("shows only descriptive recent defense validations with sign colors and no fake history", async () => { mockApi({ enhancedReport: { ...enhancedWithShanghaiAnchor, recent_defense_line_validations: [{ trading_date: "2026-08-11", source_report_id: "prior", source_report_date: "2026-08-10", defense_line_value: 3844, index_name: "上证指数", index_close: 3851, distance_points: 7, distance_pct: .18, close_position: "close_above_defense_line" }] } }); renderAt("/"); expect(await screen.findByText("近10个交易日攻防验证")).toBeInTheDocument(); expect(screen.getByText("来自 08-10 报告")).toBeInTheDocument(); expect(screen.getByText("收盘在线上")).toHaveClass("a-share-positive"); expect(screen.getByText("+7.00")).toHaveClass("a-share-positive"); expect(screen.getByText("当前已积累 1 / 10 条完整验证记录。")).toBeInTheDocument(); expect(screen.getByText(/不构成预测评分/)).toBeInTheDocument(); expect(screen.queryByText(/成功率|有效率/)).not.toBeInTheDocument(); });
  it("keeps a compact defense table and truthful zero-record state while moving risk below the main content", async () => { renderAt("/"); expect(await screen.findByText("攻防验证记录将随交易日自然积累。")).toBeInTheDocument(); expect(screen.getByText("当前已积累 0 / 10 条完整验证记录。")).toBeInTheDocument(); expect(screen.getByRole("table", { name: "攻防验证明细" })).toHaveTextContent("交易日"); const overview = document.querySelector(".report-overview-grid"); expect(overview?.children).toHaveLength(2); expect(overview?.children[0]).toHaveTextContent("近10个交易日攻防验证"); expect(overview?.children[1]).toHaveTextContent("风险提示"); const css = readFileSync(resolve(process.cwd(), "src/styles/global.css"), "utf8"); expect(css).toMatch(/\.report-overview-grid\s*\{[^}]*grid-template-columns:\s*minmax\(0,\s*1fr\)/); expect(css).toMatch(/\.proxy-recent-closes\s*\{[^}]*repeat\(10/); expect(css).toMatch(/@media \(max-width: 760px\)[\s\S]*\.proxy-recent-closes\s*\{[^}]*repeat\(5/); });
  it("renders available defense rows newest first and keeps all rows below the ten-record cap", async () => { const rows = Array.from({ length: 3 }, (_, index) => ({ trading_date: `2026-08-${String(11 - index).padStart(2, "0")}`, source_report_id: `prior-${index}`, source_report_date: `2026-08-${String(10 - index).padStart(2, "0")}`, defense_line_value: 3844, index_name: "上证指数", index_close: 3851 - index, distance_points: 7 - index * 7, distance_pct: .18 - index * .18, close_position: index === 0 ? "close_above_defense_line" as const : index === 1 ? "close_at_defense_line" as const : "close_below_defense_line" as const })); mockApi({ enhancedReport: { ...enhancedWithShanghaiAnchor, recent_defense_line_validations: rows } }); renderAt("/"); expect(await screen.findByText("当前已积累 3 / 10 条完整验证记录。")).toBeInTheDocument(); const renderedRows = Array.from(document.querySelectorAll(".defense-validation-row:not(.defense-validation-header)")); expect(renderedRows).toHaveLength(3); expect(renderedRows[0]).toHaveTextContent("08-11"); expect(renderedRows[1]).toHaveTextContent("08-10"); expect(renderedRows[2]).toHaveTextContent("08-09"); expect(screen.getByText("收盘与线相等")).toHaveClass("a-share-neutral"); expect(screen.getByText("收盘在线下")).toHaveClass("a-share-negative"); });
  it("keeps the report readable when the current index quote is unavailable without rendering false zeroes", async () => { mockApi({ enhancedReport: { ...enhancedWithShanghaiAnchor, live_market_anchor: { ...liveMarketAnchor, quote_status: "unavailable", current: null, pre_close: null, pct_change: null, quote_datetime: null, distance_points: null, distance_pct: null, defense_position: null, error_code: "remote_disconnected" } } }); renderAt("/"); expect(await screen.findByText("上证指数行情暂不可用；攻防点位不会被当作指数点位。")).toBeInTheDocument(); expect(screen.getByText("3,844.00")).toBeInTheDocument(); expect(screen.queryByText("0.00%")).not.toBeInTheDocument(); });
