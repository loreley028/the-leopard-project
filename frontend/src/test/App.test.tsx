...
const sectors: Sector[] = Array.from({ length: 66 }, (_, index) => ({
  ...
  intraday_snapshot: index === 0 ? { sector_key: "sector-1", trade_date: "2026-07-28", observed_at: "07/28 14:20", index_value: 101, pre_close: 100, pct_change: 1.26, volume: null, amount: null, provider: "eastmoney_board_spot", provider_role: "research_provider", data_status: "intraday_fresh", fetched_at: "2026-07-28T06:20:00Z" } : null,
...
const matrix: PathMatrix = { caption: "板块历史路径矩阵", dates: [{ report_id: report.id, detail_report_id: report.id, has_detailed_report: true, report_date: report.report_date!, market_as_of_date: report.market_as_of_date, market_weekday: "周五", weekday: "周日", is_weekend_report: true }], rows: [{ sector_key: "semiconductor", sector_name: "半导体", group_name: "科技硬件与通信", cells: [{ ...pathEntry, report_id: report.id, detail_report_id: report.id, has_detailed_report: true, report_date: report.report_date! }] }], status_contract: { statuses: [{ code: "hold", label: "持有", color: "#15803d", order: 1 }] } };
...
describe("Viewer research pages", () => {
  ...
  it("renders the 66-sector research table", async () => { renderAt("/sectors"); await waitFor(() => expect(screen.getByRole("table", { name: /板块研究档案/ })).toBeInTheDocument()); expect(screen.getAllByRole("row")).toHaveLength(67); });
  ...
  it("uses compact matrix headers and keeps the Sunday market date", async () => { renderAt("/reports/report-1"); const table = await screen.findByRole("table", { name: /板块历史路径矩阵/ }); const headers = Array.from(table.querySelectorAll("thead th")).map(item => item.textContent ?? ""); expect(headers.some(item => item.includes("报告"))).toBe(false); expect(headers.some(item => item.includes("7/19") && item.includes("日") && item.includes("行7/17") && item.includes("五"))).toBe(true); const css = readFileSync(resolve(process.cwd(), "src/styles/global.css"), "utf8"); expect(css).toMatch(/\.path-matrix thead th[^}]*width:\s*80px/); });
...
describe("Admin workflow and permissions", () => {
  ...
  it("renders filled interpretation fields and only one primary publish action", async () => { renderAt("/admin/reports/report-1/interpretation", { username: "admin", role: "admin" }); expect(await screen.findByText(report.core_view)).toBeInTheDocument(); expect(screen.getByText(report.market_path)).toBeInTheDocument(); expect(screen.getByRole("button", { name: "确认并发布" })).toBeInTheDocument(); expect(screen.queryByRole("button", { name: "本地解析" })).not.toBeInTheDocument(); expect(screen.getByText(/已明确匹配的 1 个板块/)).toBeInTheDocument(); });
  it("keeps raw text, all 66 paths and technical controls collapsed", async () => { renderAt("/admin/reports/report-1/interpretation", { username: "admin", role: "admin" }); expect(await screen.findByText("查看全部66个板块路径")).toBeInTheDocument(); expect(screen.getByText("高级操作")).toBeInTheDocument(); expect(screen.queryByText("fixture")).not.toBeVisible(); });
});
