...
describe("Viewer research pages", () => {
  ...
  it("shows current market assistance separately from the historical report and structures its defense line", async () => { renderAt("/"); expect(await screen.findByRole("heading", { name: report.title })).toBeInTheDocument(); expect(screen.getByText(report.core_view)).toBeInTheDocument(); expect(screen.getByText("猎豹核心判断")).toBeInTheDocument(); expect(screen.getByText("大盘观察 / 市场锚点")).toBeInTheDocument(); expect(screen.getByText("当前市场辅助")).toBeInTheDocument(); expect(screen.getByText("上个交易日收盘")).toBeInTheDocument(); expect(screen.getByText("今日上证指数")).toBeInTheDocument(); expect(screen.getAllByText("3,844.00 点")).toHaveLength(2); expect(screen.getByText("攻防线上方")).toBeInTheDocument(); expect(screen.getByText(/核心判断安全回退/)).toBeInTheDocument(); });
  it("keeps the report readable when the current index quote is unavailable without rendering false zeroes", async () => { mockApi({ enhancedReport: { ...enhancedWithShanghaiAnchor, live_market_anchor: { ...liveMarketAnchor, quote_status: "unavailable", current: null, pre_close: null, pct_change: null, quote_datetime: null, distance_points: null, distance_pct: null, defense_position: null, error_code: "remote_disconnected" } } }); renderAt("/"); expect(await screen.findByText("上证指数行情暂不可用；攻防点位不会被当作指数点位。")).toBeInTheDocument(); expect(screen.getByText("3,844.00 点")).toBeInTheDocument(); expect(screen.queryByText("0.00%")).not.toBeInTheDocument(); });
