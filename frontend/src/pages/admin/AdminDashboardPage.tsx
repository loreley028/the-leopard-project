...
const STATE: Record<string, string> = {
  pending_upload: "待上传", normally_no_report: "通常无报告", no_live: "休",
...
export function AdminDashboardPage() {
  ...
  const skip = async (day: string) => { await api.confirmNoLiveDay(day); await load(); };
  const cancel = async (day: string) => { await api.cancelNoLiveDay(day); await load(); };
  return <div className="page">
    <header><h1>管理区</h1><p>周日至周四默认待上传；周五、周六通常无报告。确认无直播只记录日历“休”，不会创建报告或板块事实。</p><nav className="admin-primary-nav" aria-label="管理区功能"><Link to="/admin">每日报告</Link><Link to="/admin/market">行情数据</Link><Link to="/admin/specifications">制作规范备份</Link><Link to="/admin/reports">系统设置 / 版本历史</Link></nav></header>
    <IslandCard title="最近两周直播日程">
      <div className="table-wrap"><table><thead><tr><th>日期</th><th>星期</th><th>状态</th><th>文件 / 版本</th><th>操作</th></tr></thead><tbody>
        {days.map(day => <tr key={day.report_date}><td>{day.report_date}</td><td>{day.weekday}</td><td>{STATE[day.state] ?? day.state}</td><td>{day.reports[0] ? `${day.reports[0].original_filename ?? day.reports[0].title} · V${day.reports[0].revision_number ?? 1}` : "—"}</td><td><Link to={`/admin/reports/new?report_date=${day.report_date}`}>{day.state === "normally_no_report" || day.state === "no_live" ? "改为上传" : "上传PDF + MD"}</Link>{" · "}{day.state === "no_live" ? <button type="button" onClick={() => void cancel(day.report_date)}>取消无直播</button> : <button type="button" onClick={() => void skip(day.report_date)}>确认无直播</button>}{day.reports[0] && <>{" · "}<Link to={`/admin/reports/${day.reports[0].id}/interpretation`}>查看解读</Link></>}</td></tr>)}
      </tbody></table></div>
    </IslandCard>
