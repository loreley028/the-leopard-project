...
export function AdminMarketPage() {
  ...
  return <div className="page"><header><p className="eyebrow">Admin · 行情运行状态</p><h1>行情数据</h1><p className="notice">研究辅助数据，无生产SLA。Viewer只读取服务器缓存；production_primary仍不存在。</p></header>
    <div className="dashboard-grid"><IslandCard title="当前Provider"><p><strong>{String(intraday?.provider ?? summary.provider ?? "尚未成功")}</strong></p><p>角色：{String(intraday?.provider_role ?? summary.provider_role ?? "research_provider")}</p><p>production_primary：不存在</p></IslandCard><IslandCard title="完整收盘数据"><p>预期完整日：{String(status.expected_latest_complete_trade_date ?? "—")}</p><p>最近成功日：{String(status.latest_complete_trade_date ?? status.latest_complete_eod ?? "尚无")}</p><p>缺失日期：{Array.isArray(status.missing_dates) && status.missing_dates.length ? status.missing_dates.join("、") : "无"}</p><p>失败Provider：{String(status.failed_provider ?? "无")}；最近重试：{String(status.last_retry_at ?? "—")}</p></IslandCard><IslandCard title="盘中缓存会话"><p>会话：{intraday?.session_status === "running" ? "运行中" : "已暂停"}；行情状态：{intradaySystemLabel(intraday)}</p><p>最近尝试：{intraday?.last_attempt_at ?? "—"}</p><p>最近有效/下次：{intraday?.latest_snapshot_at ?? "—"} / {intraday?.next_refresh_at ?? "—"}</p><p>成功/失败/延迟：{intraday?.success_count ?? 0}/{intraday?.failure_count ?? 0}/{intraday?.stale_count ?? 0}</p></IslandCard></div>
    <IslandCard title="盘中行情缓存"><p>默认每{intraday?.refresh_interval_minutes ?? 5}分钟；real_local按安全配置启动服务器会话，65个支持板块统一刷新。Viewer只读缓存，Admin可随时暂停或恢复。</p><div className="form-actions"><IslandButton disabled={busy || intraday?.session_status === "running"} onClick={() => void intradayAction("start")}>恢复盘中刷新</IslandButton><IslandButton disabled={busy || intraday?.session_status !== "running"} onClick={() => void intradayAction("pause")}>暂停盘中刷新</IslandButton><IslandButton disabled={busy} onClick={() => void intradayAction("refresh")}>立即刷新一次</IslandButton></div><p className="muted">午休、收盘后和非受控交易日不请求Provider；失败保留上一有效快照，不写0。</p></IslandCard>
    {latestIntradayRun && <IslandCard title="最近盘中 refresh run"><p><strong>{String(latestIntradayRun.run_id)}</strong></p><p>Provider：{String(latestIntradayRun.provider ?? intraday?.provider ?? "尚无")} · {String(latestIntradayRun.provider_role ?? "research_provider")}</p><p>成功/失败/延迟：{String(latestIntradayRun.success_count ?? 0)}/{String(latestIntradayRun.failure_count ?? 0)}/{String(latestIntradayRun.stale_count ?? 0)}；耗时：{latestIntradayRun.duration_ms == null ? "进行中" : `${String(latestIntradayRun.duration_ms)}ms`}</p><details><summary>查看板块结果与失败原因</summary><div className="refresh-result-list">{Array.isArray(latestIntradayRun.items) && latestIntradayRun.items.map((item: Record<string, unknown>) => <span key={String(item.sector_key)}><b>{String(item.sector_key)}</b><small>{String(item.status)}{item.detail ? ` · ${String(item.detail)}` : ""}</small></span>)}</div></details></IslandCard>}
