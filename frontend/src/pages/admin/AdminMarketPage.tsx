...
export function AdminMarketPage() {
  ...
  const [schedulerSessions, setSchedulerSessions] = useState<Array<Record<string, unknown>>>([]);
  const load = useCallback(async () => {
    const [nextSummary, nextStatus, nextIntraday, runs, nextSessions] = await Promise.all([api.marketSummary(), api.marketStatus(), api.intradayStatus(), api.marketRefreshRuns(), api.intradaySessions()]);
    ...
    setSchedulerSessions(nextSessions);
  ...
  return <div className="page"><header><p className="eyebrow">Admin · 行情运行状态</p><h1>行情数据</h1><p className="notice">研究辅助数据，无生产SLA。Viewer只读取服务器缓存；production_primary仍不存在。</p></header>
    <div className="dashboard-grid"><IslandCard title="当前Provider"><p><strong>{String(intraday?.provider ?? summary.provider ?? "尚未成功")}</strong></p><p>角色：{String(intraday?.provider_role ?? summary.provider_role ?? "research_provider")}</p><p>production_primary：不存在</p></IslandCard><IslandCard title="完整收盘数据"><p>预期完整日：{String(status.expected_latest_complete_trade_date ?? "—")}</p><p>最近成功日：{String(status.latest_complete_trade_date ?? status.latest_complete_eod ?? "尚无")}</p><p>缺失日期：{Array.isArray(status.missing_dates) && status.missing_dates.length ? status.missing_dates.join("、") : "无"}</p><p>失败Provider：{String(status.failed_provider ?? "无")}；最近重试：{String(status.last_retry_at ?? "—")}</p><p>下次补缺：{String(status.next_retry_at ?? "—")}</p></IslandCard><IslandCard title="实时运行状态"><p>自动刷新：<strong>{intraday?.admin_paused ? "已暂停" : "运行中"}</strong></p><p>市场状态：<strong>{intradaySystemLabel(intraday)}</strong></p><p>实时覆盖：{intraday?.success_count ?? 0}/65</p><p>最近刷新/下次：{intraday?.latest_snapshot_at ?? "—"} / {intraday?.next_refresh_at ?? "—"}</p><p>Provider：{intraday?.provider ?? "research_intraday_chain"}</p></IslandCard></div>
    <IslandCard title="交易日历"><p><strong>{intraday?.calendar_status === "calendar_out_of_range" ? "交易日历待更新" : intraday?.calendar_status === "calendar_unavailable" ? "交易日历异常" : "日历有效"}</strong></p><p>覆盖：{intraday?.calendar_coverage_start ?? "—"} 至 {intraday?.calendar_coverage_end ?? "—"}</p><p>来源：{intraday?.calendar_source ?? "未载入"}</p>{intraday?.calendar_warning && <p role="alert">当前日历覆盖至{intraday.calendar_coverage_end}，请安排年度更新。</p>}</IslandCard>
    <IslandCard title="Scheduler会话审计"><p>最近保留 {schedulerSessions.length} 条；过期running会话只会转为interrupted，不删除历史。</p><div className="refresh-result-list">{schedulerSessions.map(item => <span key={String(item.session_id)}><b>{String(item.status)}</b><small>{String(item.started_at)}{item.terminal_reason ? ` · ${String(item.terminal_reason)}` : item.lease_expires_at ? ` · lease至${String(item.lease_expires_at)}` : ""}</small></span>)}</div></IslandCard>
