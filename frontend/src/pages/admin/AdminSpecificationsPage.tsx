...
export function AdminSpecificationsPage() {
  ...
  return <div className="page"><header><p className="eyebrow">Admin · 低频归档</p><h1>制作规范备份</h1><p>仅用于文件备份和版本管理，不参与日报解析，也不会影响日报上传。</p></header>
    ...
    <IslandCard title={`历史版本 · ${items.length}`}><div className="table-wrap"><table><thead><tr><th>规范</th><th>版本</th><th>生效日期</th><th>文件</th><th>SHA-256</th><th>当前</th><th>操作</th></tr></thead><tbody>{items.map(item => <tr key={String(item.id)}><td>{String(item.specification_name)}</td><td>{String(item.version)}</td><td>{String(item.effective_date ?? "—")}</td><td>{String(item.original_filename)}</td><td><code>{String(item.sha256).slice(0, 12)}…</code></td><td>{item.is_current ? "是" : "否"}</td><td><a href={String(item.file_url)}>下载</a>{!item.is_current && <button type="button" onClick={async () => { await api.setCurrentSpecification(String(item.id)); await load(); }}>设为当前</button>}</td></tr>)}</tbody></table></div></IslandCard>
  </div>;
}
