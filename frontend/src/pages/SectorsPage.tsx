...
import { judgementDetail } from "../utils/judgement";
...
function LatestViewCell({ item }: { item: Sector }) {
  ...
  const viewpointContext = judgementDetail(assessment.current_path_status, assessment.current_judgement);
  ...
  return <span className="two-line-cell board-latest-view" title={title}>
    <b className={`path-chip path-${assessment.current_path_status}`}>{statusLabel}</b>
...
function PrimaryMarketCell({ item, current, marketState }: { item: Sector; current?: NonNullable<MarketCoreCurrentQuotes["sectors"]>[number]; marketState: LoadState }) {
  ...
  if (!name || !code) return <span className="two-line-cell market-enrichment-state">{marketState === "loading" ? "行情辅助加载中…" : marketState === "failed" ? "行情辅助暂不可用" : "—"}</span>;
...
export function SectorsPage() {
  const [sectorView, setSectorView] = useState<Sector[]>([]);
  const [marketRows, setMarketRows] = useState<Sector[]>([]);
  const [sectorViewState, setSectorViewState] = useState<LoadState>("loading");
  const [marketState, setMarketState] = useState<LoadState>("loading");
  const [realtimeState, setRealtimeState] = useState<LoadState>("loading");
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      const staticRequest = api.sectorView(true).then(items => {
        if (!disposed) { setSectorView(items); setSectorViewState("ready"); }
      }).catch(() => { if (!disposed) setSectorViewState("failed"); throw new Error("sector_view_failed"); });
      const marketRequest = api.sectors(true).then(items => {
        if (!disposed) { setMarketRows(items); setMarketState("ready"); }
      }).catch(() => { if (!disposed) setMarketState("failed"); throw new Error("sector_market_failed"); });
      const realtimeRequest = api.marketCurrent("matrix").then(current => {
        if (!disposed) { setCurrentQuotes(current); setRealtimeState("ready"); }
      }).catch(() => { if (!disposed) { setCurrentQuotes(undefined); setRealtimeState("failed"); } throw new Error("realtime_failed"); });
      const auxiliaryRequests = [
        api.intradayStatus().then(status => { if (!disposed) setIntraday(status); }),
        api.me().then(me => { if (!disposed) setPrincipal(me); }).catch(() => undefined),
      ];
      const results = await Promise.allSettled([staticRequest, marketRequest, realtimeRequest, ...auxiliaryRequests]);
      failures = results.some(item => item.status === "rejected") ? Math.min(3, failures + 1) : 0;
  ...
  const marketBySector = useMemo(() => new Map(marketRows.map(item => [item.sector_key, item])), [marketRows]);
  const sectors = useMemo(() => sectorView.map(item => {
    const enrichment = marketBySector.get(item.sector_key);
    if (!enrichment) return item;
    return {
      ...item,
      latest_market: enrichment.latest_market,
      primary_market: enrichment.primary_market,
      latest_complete_market: enrichment.latest_complete_market,
      intraday_snapshot: enrichment.intraday_snapshot,
      intraday_status: enrichment.intraday_status,
      intraday_last_attempt_at: enrichment.intraday_last_attempt_at,
      recent_10_trading_days: enrichment.recent_10_trading_days,
      active_holding_interval: enrichment.active_holding_interval,
      historical_holding_intervals: enrichment.historical_holding_intervals,
      strict_holding_interval: enrichment.strict_holding_interval,
      broad_holding_interval: enrichment.broad_holding_interval,
      historical_strict_intervals: enrichment.historical_strict_intervals,
      historical_broad_intervals: enrichment.historical_broad_intervals,
    };
  }), [sectorView, marketBySector]);
  ...
  const activeFilterCount = Number(showDormant) + [group, path, mentioned, market].filter(value => value !== "all").length;
  ...
  const togglePin = async (item: Sector) => { if (item.is_pinned_for_research) await api.unpinSector(item.sector_key); else await api.pinSector(item.sector_key); setSectorView(await api.sectorView(true)); };
  return <div className="page sectors-research"><header><h1>板块研究</h1><p>74个显示对象中包含71个active Report Object；行情观察始终使用固定主标的，恒生科技保持跨市场不支持。</p></header>
    ...
    <div className={`sector-filter-area ${mobileFiltersOpen ? "mobile-filters-open" : ""}`}>
      ...
      <div className="sector-filters" aria-label="板块筛选"><label className="sector-search-filter">搜索<input value={search} onChange={event => setSearch(event.target.value)} /></label><button className={activeFilterCount ? "mobile-filter-toggle active" : "mobile-filter-toggle"} type="button" aria-expanded={mobileFiltersOpen} aria-controls="sector-advanced-filters" onClick={() => setMobileFiltersOpen(value => !value)}>筛选{activeFilterCount ? ` · ${activeFilterCount}` : ""}<span aria-hidden="true">{mobileFiltersOpen ? "▲" : "▼"}</span></button><div id="sector-advanced-filters" className="sector-advanced-filters"><label>一级分组<select value={group} onChange={event => setGroup(event.target.value)}><option value="all">全部</option>{groups.map(item => <option key={item.group_order}>{item.group_name}</option>)}</select></label><label>路径状态<select value={path} onChange={event => setPath(event.target.value)}><option value="all">全部</option><option value="hold">持有</option><option value="watch">观察</option><option value="not_mentioned">未提</option></select></label><label>本期提及<select value={mentioned} onChange={event => setMentioned(event.target.value)}><option value="all">全部</option><option value="true">已提及</option><option value="false">未提及</option></select></label><label>行情状态<select value={market} onChange={event => setMarket(event.target.value)}><option value="all">全部</option><option value="supported">支持</option><option value="proxy">代理</option><option value="short_history">短历史</option><option value="unsupported">不支持</option></select></label></div></div>
    </div>
    ...
    {sectorViewState === "failed" && <p className="data-load-notice error">报告观点暂时无法加载，请稍后重试。</p>}
    {marketState === "failed" && <p className="data-load-notice">报告观点已加载；行情辅助暂不可用。</p>}
    {realtimeState === "failed" && <p className="data-load-notice">实时行情暂不可用；报告观点与完整交易日数据不受影响。</p>}
    <div className="sector-table-wrap table-wrap"><table className="sector-table"><colgroup><col className="sector-name-column" /><col className="sector-group-column" /><col className="sector-status-column" /><col className="sector-path-column" /><col className="sector-view-column" /><col className="sector-market-column" /><col className="sector-return-column" /><col className="sector-detail-column" /><col className="sector-holding-column" /><col className="sector-ma-column" /><col className="sector-action-column" /></colgroup><caption>板块研究档案（{sectorViewState === "loading" ? "报告观点加载中" : `当前${filtered.length}个显示对象`}；71个active Report Object）</caption><thead><tr><th>板块</th><th>分组</th><th title="本期报告状态 / 延续后的有效状态">本期/有效</th><th>最近10期</th><th>最新观点</th><th>当前/最近行情</th><th>近10日</th><th>查看10日</th><th>持有区间</th><th>MA关系</th><th>操作</th></tr></thead><tbody>{filteredGroups.flatMap(({ group_name: groupName, rows }) => [<tr className="sector-group-row" data-group-name={groupName} ref={node => { if (node) groupTargets.current.set(groupName, node); else groupTargets.current.delete(groupName); }} key={`group-${groupName}`}><th className="sector-group-name" scope="rowgroup">{groupName}<small>{rows.length}项</small></th><td colSpan={2} aria-hidden="true" /><td className="group-recent10-axis"><GroupReportDateAxis rows={rows} /></td><td colSpan={7} aria-hidden="true" /></tr>, ...rows.map(item => <tr className="sector-card-row" key={item.sector_key}><th className="sector-name-cell" scope="row"><Link to={`/sectors/${item.sector_key}`}>{item.sector_name}</Link>{item.status_changed && <small className="changed-mark">状态变化</small>}{principal?.role === "admin" && <button className="pin-sector" type="button" onClick={() => void togglePin(item)}>{item.is_pinned_for_research ? "取消常驻" : "常驻关注"}</button>}</th><td>{item.group_name}</td>
<td><span className="status-pair" data-statuses-match={item.current_path_status === item.effective_status}><b className={`mobile-card-status path-chip path-${item.effective_status ?? item.current_path_status}`}>{item.effective_status_label ?? item.current_path_status_label}</b><span className="status-current-detail"><small>本期</small><b>{item.current_path_status_label}</b></span><span className="status-effective-detail"><small>有效</small><b>{item.effective_status_label ?? "暂无"}</b></span>{item.effective_source_report_date && <small className="effective-source">来源 {item.effective_source_report_date.slice(5)}{item.effective_derived_from_transition ? " · 由转持延续" : ""}</small>}</span></td>
...
<td><PrimaryMarketCell item={item} current={currentBySector.get(item.sector_key)} marketState={marketState} /></td>
<td>{marketState === "loading" ? "加载中…" : marketState === "failed" ? "—" : <PrimaryTenDay item={item} />}</td>
<td>{marketState === "loading" ? "加载中…" : marketState === "failed" ? "—" : <PrimaryTenDayDetails item={item} />}</td>
<td>{marketState === "loading" ? "加载中…" : marketState === "failed" ? "—" : <span className="two-line-cell holding-cell" title={`绝对：${item.strict_holding_interval?.start_report_date ?? "—"}起；广义：${item.broad_holding_interval?.start_report_date ?? "—"}起`}><small>绝对 {item.strict_holding_interval?.status === "active" ? shortDate(item.strict_holding_interval.start_report_date) + " → 至今" : "—"}</small><small>广义 {item.broad_holding_interval?.status === "active" ? shortDate(item.broad_holding_interval.start_report_date) + " → 至今" : "—"}</small></span>}</td>
<td>{marketState === "loading" ? "加载中…" : marketState === "failed" ? <span className="market-enrichment-state">行情辅助暂不可用</span> : item.primary_market?.close != null ? <span className="two-line-cell ma-cell"><small className={marketTone(item.primary_market.close_vs_ma5_pct)}>{maLine("MA5", item.primary_market.close_vs_ma5_pct)}</small><small className={marketTone(item.primary_market.close_vs_ma10_pct)}>{maLine("MA10", item.primary_market.close_vs_ma10_pct)}</small><small className={marketTone(item.primary_market.close_vs_ma20_pct)}>{maLine("MA20", item.primary_market.close_vs_ma20_pct)}</small></span> : "—"}</td>
