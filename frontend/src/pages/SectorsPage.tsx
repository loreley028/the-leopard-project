...
export function SectorsPage() {
  ...
  return <div className="page sectors-research"><header><h1>板块研究</h1><p>66个报告主题保持不变；行情研究使用67条路径，其中66条支持、恒生科技另计不支持。</p></header>
    ...
    <div className="sector-table-wrap table-wrap"><table className="sector-table"><caption>板块研究档案（当前{filtered.length}个报告主题；底层行情路径67条）</caption><thead><tr><th>板块</th><th>分组</th><th title="本期报告状态 / 延续后的有效状态">本期/有效</th><th>最近10期</th><th>最新观点</th><th>当前/最近行情</th><th>近10日</th><th>查看10日</th><th>持有区间</th><th>MA关系</th><th>操作</th></tr></thead><tbody>{filteredGroups.flatMap(({ group_name: groupName, rows }) => [<tr className="sector-group-row" data-group-name={groupName} ref={node => { if (node) groupTargets.current.set(groupName, node); else groupTargets.current.delete(groupName); }} key={`group-${groupName}`}><th colSpan={11}>{groupName}<small>{rows.length}项</small><GroupReportDateAxis rows={rows} /></th></tr>, ...rows.map(item => <tr key={item.sector_key}><th scope="row"><Link to={`/sectors/${item.sector_key}`}>{item.sector_name}</Link>{item.parent_report_topic === "hotel_catering" && <small>行情辅助：酒店路径</small>}{item.status_changed && <small className="changed-mark">状态变化</small>}{principal?.role === "admin" && <button className="pin-sector" type="button" onClick={() => void togglePin(item)}>{item.is_pinned_for_research ? "取消常驻" : "常驻关注"}</button>}</th><td>{item.group_name}</td><td><span className="status-pair"><small>本期</small><b>{item.current_path_status_label}</b><small>有效</small><b>{item.effective_status_label ?? "暂无"}</b></span></td><td><span className="mini-path-strip">{(item.recent_path ?? []).slice(-10).map(entry => <i key={`${entry.report_id}-${entry.report_date}`} className={`path-${entry.path_status}`} title={`${entry.report_date} ${entry.path_status_label}`}><b>{entry.path_status_label.slice(0, 1)}</b></i>)}</span></td><td><span className="two-line-cell" title={item.latest_view ?? "暂无明确观点"}><b>{item.current_path_status_label}</b><small>{shortDate(item.latest_view_date)}</small></span></td><td><PrimaryMarketCell item={item} /></td><td><PrimaryTenDay item={item} /></td><td><PrimaryTenDayDetails item={item} /></td><td><span className="two-line-cell holding-cell" title={`绝对：${item.strict_holding_interval?.start_report_date ?? "—"}起；广义：${item.broad_holding_interval?.start_report_date ?? "—"}起`}><small>绝对 {item.strict_holding_interval?.status === "active" ? shortDate(item.strict_holding_interval.start_report_date) + " → 至今" : "—"}</small><small>广义 {item.broad_holding_interval?.status === "active" ? shortDate(item.broad_holding_interval.start_report_date) + " → 至今" : "—"}</small></span></td><td>{item.primary_market?.close != null ? <span className="two-line-cell ma-cell"><small className={marketTone(item.primary_market.close_vs_ma5_pct)}>{maLine("MA5", item.primary_market.close_vs_ma5_pct)}</small><small className={marketTone(item.primary_market.close_vs_ma10_pct)}>{maLine("MA10", item.primary_market.close_vs_ma10_pct)}</small><small className={marketTone(item.primary_market.close_vs_ma20_pct)}>{maLine("MA20", item.primary_market.close_vs_ma20_pct)}</small></span> : "—"}</td><td><Link className="sector-row-action" to={`/sectors/${item.sector_key}`}>查看</Link></td></tr>)])}</tbody></table></div>
  </div>;
}
