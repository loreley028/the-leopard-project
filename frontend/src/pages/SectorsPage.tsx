...
/**
 * The Reader catalog is report-topic-first: hotel and catering are two market
 * paths beneath the single "酒店餐饮" report topic.  Keep the path with the
 * most useful market overlay while rendering one stable report row.
 */
function reportTopicRows(items: Sector[]): Sector[] {
  const byTopic = new Map<string, Sector>();
  const rank = (item: Sector) => item.data_status === "proxy" ? 2 : item.data_status === "supported" ? 1 : 0;
  for (const item of items) {
    const topic = item.parent_report_topic ?? item.sector_key;
    const current = byTopic.get(topic);
    if (!current || rank(item) > rank(current)) {
      byTopic.set(topic, item);
    }
  }
  return [...byTopic.values()].map(item => item.parent_report_topic
    ? { ...item, sector_name: item.report_topic_name ?? item.sector_name }
    : item,
  );
}
...
function RecentTen({ days }: { days: RecentTradingDay[] | undefined }) {
  ...
  return <span className="recent-five-mini" aria-label="最近10个交易日逐日涨跌">
    {days.map(item => <i key={item.trade_date} className={item.daily_pct_change > 0 ? "up" : item.daily_pct_change < 0 ? "down" : "flat"} title={`${item.trade_date} ${formatPct(item.daily_pct_change)} 收盘 ${item.close}`}><small>{shortDate(item.trade_date)}</small><b>{formatPct(item.daily_pct_change)}</b><em>收盘 {item.close.toFixed(2)}</em></i>)}
  </span>;
}
...
export function SectorsPage() {
  ...
  const [visibility, setVisibility] = useState<"all" | "low">("all");
  ...
  const reportTopics = useMemo(() => reportTopicRows(sectors), [sectors]);
  ...
  const filtered = useMemo(() => reportTopics.filter(item =>
    ...
    (visibility !== "low" || item.is_low_attention) &&
  ...
  ), [reportTopics, search, visibility, group, path, mentioned, market, sort]);
  ...
  const hiddenCount = reportTopics.filter(item => item.is_low_attention).length;
  ...
  return <div className="page sectors-research"><header><h1>板块研究</h1><p>66个报告主题保持不变；行情研究使用67条路径，其中66条支持、恒生科技另计不支持。</p></header>
    ...
    <div className="visibility-toolbar"><strong>低关注主题：{hiddenCount}个</strong><button type="button" onClick={() => setVisibility("all")} className={visibility === "all" ? "active" : ""}>全部报告主题</button><button type="button" onClick={() => setVisibility("low")} className={visibility === "low" ? "active" : ""}>仅低关注</button><small>66个报告主题始终保留；行情仅作为可选辅助信息。</small></div>
    ...
    <div className="sector-table-wrap table-wrap"><table className="sector-table"><caption>板块研究档案（当前{filtered.length}个报告主题；底层行情路径67条）</caption><thead><tr><th>板块</th><th>分组</th><th title="本期报告状态 / 延续后的有效状态">本期/有效</th><th>最近10期</th><th>最新观点</th><th>当前/最近行情</th><th>近10日</th><th>查看10日</th><th>持有区间</th><th>MA关系</th><th>操作</th></tr></thead><tbody>{filteredGroups.flatMap(({ group_name: groupName, rows }) => [<tr className="sector-group-row" data-group-name={groupName} ref={node => { if (node) groupTargets.current.set(groupName, node); else groupTargets.current.delete(groupName); }} key={`group-${groupName}`}><th colSpan={11}>{groupName}<small>{rows.length}项</small></th></tr>, ...rows.map(item => <tr key={item.sector_key}><th scope="row"><Link to={`/sectors/${item.sector_key}`}>{item.sector_name}</Link>{item.parent_report_topic === "hotel_catering" && <small>行情辅助：酒店路径</small>}{item.status_changed && <small className="changed-mark">状态变化</small>}{principal?.role === "admin" && <button className="pin-sector" type="button" onClick={() => void togglePin(item)}>{item.is_pinned_for_research ? "取消常驻" : "常驻关注"}</button>}</th><td>{item.group_name}</td><td><span className="status-pair"><small>本期</small><b>{item.current_path_status_label}</b><small>有效</small><b>{item.effective_status_label ?? "暂无"}</b></span></td><td><span className="mini-path-strip">{(item.recent_path ?? []).slice(-10).map(entry => <i key={`${entry.report_id}-${entry.report_date}`} className={`path-${entry.path_status}`} title={`${entry.report_date} ${entry.path_status_label}`}>{entry.path_status_label.slice(0, 1)}</i>)}</span></td><td><span className="two-line-cell" title={item.latest_view ?? "暂无明确观点"}><b>{item.current_path_status_label}</b><small>{shortDate(item.latest_view_date)}</small></span></td><td><PrimaryMarketCell item={item} /></td><td><PrimaryTenDay item={item} /></td><td><PrimaryTenDayDetails item={item} /></td><td><span className="two-line-cell holding-cell" title={`绝对：${item.strict_holding_interval?.start_report_date ?? "—"}起；广义：${item.broad_holding_interval?.start_report_date ?? "—"}起`}><small>绝对 {item.strict_holding_interval?.status === "active" ? formatPct(item.strict_holding_interval.eod_return) : "—"}</small><small>广义 {item.broad_holding_interval?.status === "active" ? formatPct(item.broad_holding_interval.eod_return) : "—"}</small></span></td><td>{item.primary_market?.close != null ? <span className="two-line-cell ma-cell"><small className={marketTone(item.primary_market.close_vs_ma5_pct)}>{maLine("MA5", item.primary_market.close_vs_ma5_pct)}</small><small className={marketTone(item.primary_market.close_vs_ma10_pct)}>{maLine("MA10", item.primary_market.close_vs_ma10_pct)}</small><small className={marketTone(item.primary_market.close_vs_ma20_pct)}>{maLine("MA20", item.primary_market.close_vs_ma20_pct)}</small></span> : "—"}</td><td><Link className="sector-row-action" to={`/sectors/${item.sector_key}`}>查看</Link></td></tr>)])}</tbody></table></div>
  </div>;
}
