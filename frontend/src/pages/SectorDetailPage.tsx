...
import type { MarketCoreProxies, SectorResearch, TimelinePathEntry, ViewerObservation } from "../types";
...
export function SectorDetailPage() {
  ...
  const [selectedTimeline, setSelectedTimeline] = useState<TimelinePathEntry | null>(null);
  ...
  const primaryMarket = research.primary_market ?? null;
  const timelineMarketBasis = research.timeline_market_basis ?? null;
  const recentTradingDays = primaryMarket?.history ?? research.recent_10_trading_days ?? [];
  const tenDayReturn = availablePeriodReturn(recentTradingDays, primaryMarket?.return_10d);
  const selectedOverlay = selectedTimeline?.market_overlay;
  const latestViewDetail = research.latest_explicit_view ? judgementDetail(research.latest_explicit_view.assessment.current_path_status, research.latest_explicit_view.assessment.current_judgement) : "";
  return <div className="page sector-research-detail"><header><p className="eyebrow">{research.group_name}</p><h1>{research.sector_name}</h1><IslandStatusBadge status={research.data_status} /><p>{research.market_status_detail}</p>{research.parent_report_topic === "hotel_catering" && <div className="notice"><strong>上层报告主题：酒店餐饮</strong><p>报告观点保持一条，不复制为两个结论；酒店与餐饮只拆分实时行情研究。</p><p><Link to="/sectors/hotel">酒店行情</Link> · <Link to="/sectors/catering">餐饮行情</Link></p></div>}<div className="path-period-switch" aria-label="直播报告路径期数">{[10,20,40,60].map(value => <button type="button" key={value} className={pathPeriods === value ? "active" : ""} onClick={() => setPathPeriods(value)}>最近{value}期</button>)}</div><p className="muted">路径范围按直播报告期计算；实际显示 {research.recent_path?.length ?? 0} / 可用 {research.available_path_periods ?? 0} 期。无原始PDF的日期只显示冻结路径记录。</p><p className="sector-timeline-market-basis">{timelineMarketBasis ? <>行情口径：<strong>{timelineMarketBasis.name} · {timelineMarketBasis.security_code}</strong>；历史涨跌均为对应完整交易日收盘表现。</> : "暂无可靠固定主观察标的行情"}</p><div className="path-month-groups">{monthGroups.map(group => <section key={group.month}><strong>{group.month}</strong><div className="recent-path-banner">{group.items.map((item, index) => { const overlay = item.market_overlay; const marketDateDiffers = overlay?.kind === "primary" && overlay.market_date != null && overlay.market_date !== item.report_date; return <button type="button" key={item.id} className={`sector-timeline-tile ${index === group.items.length - 1 && group === monthGroups.at(-1) ? "latest" : ""}`} onClick={() => setSelectedTimeline(item)} title={`${item.report_date} · 报告状态${item.path.path_status_label} · 有效状态${item.effective_status ? PATH_LABELS[item.effective_status] : "暂无"} · 行情日${overlay?.market_date ?? "暂无"}`}><span className={`sector-timeline-report path-${item.path.path_status}`}><small>{item.report_date.slice(5)}</small><b>{item.path.path_status_label.slice(0, 1)}</b></span><span className="sector-timeline-market">{overlay?.kind === "primary" ? <>{marketDateDiffers && <small>行{overlay.market_date?.slice(5)}</small>}<em className={aShareTone(overlay.pct_change)}>{formatPct(overlay.pct_change)}</em></> : "—"}</span></button>; })}</div></section>)}</div></header>
    {selectedTimeline && <section className="sector-timeline-detail" aria-label="路径与行情详情"><strong>路径与行情详情 · {selectedTimeline.report_date}</strong><p>报告状态：{selectedTimeline.path.path_status_label}；行情日：{selectedOverlay?.market_date ?? "—"}。</p>{selectedOverlay?.kind === "primary" && selectedOverlay.primary ? <><p><strong>主观察标的</strong></p><ul className="sector-timeline-security-list"><li><span>{selectedOverlay.primary.name} · {selectedOverlay.primary.security_code}<small> · {selectedOverlay.primary.role === "etf" ? "代理ETF" : "核心公司"}</small></span><b className={aShareTone(selectedOverlay.primary.pct_change)}>{formatPct(selectedOverlay.primary.pct_change)}</b></li>{selectedOverlay.instruments.map(item => <li key={`${item.security_code}-${item.trading_date}`}><span>{item.name} · {item.security_code}<small> · {item.role === "etf" ? "代理ETF" : "核心公司"}</small></span><b className={aShareTone(item.pct_change)}>{formatPct(item.pct_change)}</b></li>)}</ul><p className="muted">逐项为对应行情日的精确表现，不代表板块指数或综合收益。</p></> : <p className="muted">该报告映射的交易日暂无可靠主观察标的行情。</p>}</section>}
    <div className="dashboard-grid">
      <IslandCard title="报告观点">{research.latest_explicit_view ? <>{research.latest_report_explicitly_mentioned === false && <p className="muted">本期报告（{research.latest_report_date ?? "最新一期"}）未提及；沿用最近明确观点。</p>}<strong>{research.latest_explicit_view.path.path_status_label} · {research.latest_explicit_view.report_date}</strong>{latestViewDetail && <p>{latestViewDetail}</p>}<p><Link to={`/reports/${research.latest_explicit_view.report_id}`}>查看来源报告</Link></p></> : <p>暂无明确直播观点；“未提”不代表观点失效。</p>}</IslandCard>
      {officialBoardAvailable && <IslandCard title="当前官方板块行情"><p>状态：<strong>{intradayLabel}</strong></p>{research.intraday_snapshot ? <dl className="intraday-detail-list"><div><dt>当前值</dt><dd>{research.intraday_snapshot.index_value}</dd></div><div><dt>昨收</dt><dd>{research.intraday_snapshot.pre_close}</dd></div><div><dt>实时涨跌</dt><dd>{formatPct(research.intraday_snapshot.pct_change)}</dd></div><div><dt>实时MA5</dt><dd>{research.intraday_snapshot.intraday_ma5 ?? "同源历史不足"}</dd></div><div><dt>相对实时MA5</dt><dd>{formatPct(research.intraday_snapshot.intraday_vs_ma5)}</dd></div><div><dt>行情时间</dt><dd>{research.intraday_snapshot.observed_at}</dd></div><div><dt>数据状态</dt><dd>{intradayLabel}</dd></div></dl> : <p>保留服务器最近有效缓存；Viewer不会请求Provider。</p>}<p className="reader-provider-note">数据说明：此处为官方板块行情辅助；完整日历史另按固定主观察证券的 Market Core 数据展示。</p></IslandCard>}
      <IslandCard title="固定主观察 · 最近完整收盘行情">{primaryMarket?.close != null ? <><p>{primaryMarket.name} · {primaryMarket.security_code}</p><p>完整交易日：{primaryMarket.trade_date}</p><dl className="intraday-detail-list"><div><dt>收盘</dt><dd>{formatSecurityPrice(primaryMarket.close)}</dd></div><div><dt>日涨跌</dt><dd className={aShareTone(primaryMarket.daily_pct_change)}>{formatPct(primaryMarket.daily_pct_change)}</dd></div><div><dt>MA5</dt><dd>{formatSecurityPrice(primaryMarket.ma5)}</dd></div><div><dt>MA10</dt><dd>{formatSecurityPrice(primaryMarket.ma10)}</dd></div><div><dt>MA20</dt><dd>{formatSecurityPrice(primaryMarket.ma20)}</dd></div></dl></> : <p>暂无可靠主观察证券；不以旧板块历史冒充当前行情。</p>}</IslandCard>
    </div>
    ...
    <IslandCard title="最近10个交易日">{primaryMarket?.name && <p className="muted">固定主观察：{primaryMarket.name} · {primaryMarket.security_code}</p>}<p>10日累计：<strong>{formatPct(tenDayReturn)}</strong>（按完整日收益复合/首尾价格计算，不作百分比简单相加）</p>{recentTradingDays.length > 0 && recentTradingDays.length < 10 && <p className="muted">当前仅有 {recentTradingDays.length} 个完整交易日</p>}<div className="recent-five-detail">{recentTradingDays.length ? recentTradingDays.map(item => <span key={item.trading_date} className={item.daily_pct_change != null && item.daily_pct_change > 0 ? "up" : item.daily_pct_change != null && item.daily_pct_change < 0 ? "down" : "flat"}><time>{item.trading_date}</time><b>{formatPct(item.daily_pct_change)}</b><small>收盘 {item.close}</small></span>) : <p>暂无完整行情。</p>}</div></IslandCard>
    ...
    <section><h2>历次详细解读与发布快照</h2><div className="assessment-list">{research.detailed_history?.map(item => { const detail = judgementDetail(item.assessment.current_path_status, item.assessment.current_judgement); return <IslandCard key={item.report_id}><h3>{item.report_date} · {item.path.path_status_label}</h3><dl className="assessment-fields"><div><dt>历史路径</dt><dd>{item.assessment.recent_path_summary}</dd></div><div><dt>当期判断</dt><dd>{detail || item.path.path_status_label}</dd></div><div><dt>主要依据</dt><dd>{item.assessment.main_basis || "—"}</dd></div><div><dt>观察条件</dt><dd>{item.assessment.observation_condition || "—"}</dd></div></dl><h4>本报告发布时</h4><IslandMetricGrid market={item.report_snapshot} /><Link to={`/reports/${item.report_id}`}>来源报告</Link></IslandCard>; })}</div></section>
