...
export function SecurityProxyCard({ observation }: { observation: ViewerObservation }) {
  ...
  return <IslandCard title="代理观察"><p><strong className="proxy-observation-badge">代理观察</strong> 正式板块行情暂不可用。</p>{observation.selection_mode === "dynamic_eod_snapshot" && <p className="muted">今日观察名单根据上一交易日日终数据更新。</p>}{observation.selection_mode === "static_approved_registry" && <p className="muted">人工批准名单；部分指标数据不足时保持固定观察名单。</p>}<div className="proxy-observation-list">{proxy.instruments.map(item => <div key={item.symbol}><strong>{item.proxy_role === "etf" ? "代表ETF" : "核心公司"} · {item.security_name}</strong>{item.coverage_type === "partial" && <small>部分覆盖</small>}<span>{item.quote_status === "available" ? <>{formatPct(item.pct_change)} 现价：{item.current} 行情时间：{item.quote_datetime?.slice(11, 16)}</> : "行情暂不可用"}</span></div>)}</div><p className="notice">{observation.disclosure}</p></IslandCard>;
}
