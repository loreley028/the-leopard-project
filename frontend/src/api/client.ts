...
export const api = {
  ...
  interpret: (file: File, reportDate?: string, mdFile?: File) => { const body = new FormData(); body.append("file", file); if (mdFile) body.append("md_file", mdFile); if (reportDate) body.append("report_date_hint", reportDate); return request<{ report: Report; interpretation: Interpretation; duplicate: boolean; publication: "published" | "already_published" | "needs_review"; interpretation_error: { code: string; message: string } | null; md_validation: { schema: string; schema_version: string; report_date: string; display_row_count: number; active_object_count: number; updated_sector_count: number; unmentioned_sector_count: number; updated_plus_unmentioned: number; validation_status: string } | null; processing_steps: string[] }>("/admin/reports/interpret", { method: "POST", body }); },
