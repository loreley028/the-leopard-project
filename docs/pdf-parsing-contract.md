...
## Template routing through V2.4

V2.3, V2.3.1 and V2.4 are first-class routes. V2.4 uses PDF text, layout and coordinates without OCR or an external LLM. The historical matrix is isolated from the detailed table; the explicit 当日判断 cell is authoritative and explanatory basis/condition text never reclassifies it. B3 can cover the remaining catalog through the 本场未更新 list. Frozen history is compared through the declared freeze date and only the report-date column is appended.
