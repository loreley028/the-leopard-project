...
结果页先显示自动确认、建议检查和必须处理三类数量。每个疑问使用稳定`issue_key`，直接显示系统识别、系统建议、可选结果和当前页PDF证据；技术代码默认折叠。接受建议、人工选择和批量接受分别留存来源、最终值、操作人和时间。重复提交幂等，重新解析不会覆盖人工决定，已发布报告的确认记录只读。

全部66板块仍以`not_mentioned`等状态保存，但只在“查看全部66个板块路径”折叠区展示。原始文本、完整路径和解析诊断位于默认收起的“高级技术信息”。
