...
def register_enhanced_routes(
    ...
    def enhanced_report(report_id: str, current: Principal | None = Depends(optional_principal), session: Session = Depends(db_session)) -> dict:
        ...
        reader_keys = {
            ...
            if is_active_report_object_on(item.sector_key, report.report_date)
        }
        assessments = []
        for item in service.assessments(report.id):
            if item.sector_key not in reader_keys:
                continue
        ...
        paths = [path_entry_payload(item) for item in service.path_entries(report.id) if item.sector_key in reader_keys]
        ...
        return {
            ...
            "market_snapshots": [item for key, item in snapshots.items() if key in reader_keys],
    ...
    def path_matrix(
        ...
        entries = [item for item in session.scalars(select(SectorPathHistoryEntry).where(
            ...
            item.sector_key in reader_keys
            and is_active_report_object_on(item.sector_key, item.path_report_date)
        ...
        for published_report in published_reports:
            ...
            for path in report_paths:
                if (
                    path.sector_key not in reader_keys
                    or not is_active_report_object_on(path.sector_key, published_report.report_date)
                ):
                    continue
            ...
            for assessment in service.assessments(published_report.id):
                if (
                    assessment.sector_key not in reader_keys
                    or assessment.sector_key in path_keys
                    or not is_active_report_object_on(assessment.sector_key, published_report.report_date)
                ):
                    continue
                fallback_entries.append(SimpleNamespace(
        ...
        dormant_by_sector = {
            sector.sector_key: classify_dormant_sector(
                (entry for entry in entries if entry.sector_key == sector.sector_key),
                all_trading_dates,
            )
        ...
        rows = []
        for sector in report_objects:
            dormant = dormant_by_sector[sector.sector_key]
            rows.append({
                "sector_key": sector.sector_key,
                "sector_name": sector.sector_name,
                ...
                "market_available": sector.sector_key in proxy_definitions,
                "is_dormant_20d": sector.lifecycle == "active" and dormant.is_dormant,
    ...
    def report_assessments(report_id: str, current: Principal | None = Depends(optional_principal), session: Session = Depends(db_session)) -> list[dict]:
        ...
        return [assessment_payload(item, snapshots.get(item.sector_key)) for item in service.assessments(report.id) if item.sector_key in reader_keys]
    ...
    def sector_research(
        ...
        if report_object.lifecycle != "active":
            raise WebDomainError("sector_not_found", "Sector not found", 404)
        ...
        for report in reports:
            if not is_active_report_object_on(report_key, report.report_date):
                continue
        ...
        pref = session.get(SectorResearchPreference, report_key)
        is_pinned = bool(pref and pref.is_pinned_for_research)
        last_ten = all_path_rows[:10]
        data_status = "unsupported" if unsupported else "proxy" if primary_definition is not None else "short_history" if report_key == "glass_substrate" else "unverified" if capability and not capability.selectable_candidates else "supported"
        dormant = classify_dormant_sector(all_path_rows, market_core_completed_dates(session))
