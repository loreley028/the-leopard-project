...
from leopard_project.web.intraday import IntradayRefreshCoordinator, market_phase, resolve_intraday_data_status
...
def daily_bar(sector_key: str, day: date, close: Decimal = Decimal("101")) -> DailyBar:
    return DailyBar(
        ...
        liquidity_status=LiquidityStatus.PARTIAL, provider="synthetic_intraday_provider",
...
def test_intraday_is_one_server_cache_and_never_enters_eod_models(tmp_path: Path) -> None:
    ...
    with TestClient(app) as client:
        ...
        assert run_detail["provider"] == "eastmoney_board_spot"
...
def test_intraday_break_close_and_overlap_never_request_provider(tmp_path: Path) -> None:
    ...
    for now, expected in (
        ...
        (datetime(2026, 7, 26, 2, 0, tzinfo=timezone.utc), "market_closed"),
    ):
